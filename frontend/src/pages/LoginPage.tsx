  ...
  return (
    <div className="flex items-center gap-4 group/logo cursor-pointer">
      <div className="relative">
        <div className={`absolute inset-0 blur-2xl rounded-full opacity-0 group-hover/logo:opacity-50 transition-opacity duration-1000 ${isDark ? 'bg-rose-500/40' : 'bg-blue-500/30'}`} />
        <SfmLogoIcon className={`${iconSize} relative z-10 transition-transform duration-500 group-hover/logo:scale-105 drop-shadow-xl`} />
      </div>
      <div className="flex flex-col">
        <span className={`${titleSize} font-black tracking-tight leading-none transition-colors duration-1000 ${isDark ? 'text-white' : 'text-slate-900'}`}>
          SFM
        </span>
        <span className={`${subSize} font-bold tracking-widest uppercase text-rose-600 transition-colors duration-1000`}>
          Spartans
        </span>
      </div>
    </div>
...
const LoginPage = () => {
  ...
  return (
    <div className={`min-h-screen flex selection:bg-rose-500/30 transition-colors duration-1000 ${isDark ? 'bg-[#0a0e1a]' : 'bg-white'}`}>
      
      {/* ═══════════════════ LEFT SIDE — Branding Panel ═══════════════════ */}
      <div className="hidden lg:flex lg:w-1/2 relative overflow-hidden">

        {/* ── Theme Crossfade Backgrounds (For Buttery Smooth Transitions) ── */}
        <div className={`absolute inset-0 bg-gradient-to-br from-white via-slate-50 to-rose-50/40 transition-opacity duration-1000 ease-in-out ${isDark ? 'opacity-0' : 'opacity-100'}`} />
        <div className={`absolute inset-0 bg-gradient-to-br from-[#0f1629] via-[#0a0e1a] to-[#0d1117] transition-opacity duration-1000 ease-in-out ${isDark ? 'opacity-100' : 'opacity-0'}`} />
        ...
        />
        <div
          className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${isDark ? 'opacity-0' : 'opacity-100'}`}
        ...
        <motion.div
          ...
          className={`absolute -top-24 -left-24 w-[520px] h-[520px] rounded-full blur-[100px] transition-colors duration-1000 ${isDark ? 'bg-blue-600/15' : 'bg-rose-400/10'}`}
        />
        <motion.div
          animate={{ x: [0, -25, 20, 0], y: [0, 25, -15, 0] }}
          transition={{ duration: 22, repeat: Infinity, ease: 'easeInOut' }}
          className={`absolute -bottom-32 -right-20 w-[480px] h-[480px] rounded-full blur-[100px] transition-colors duration-1000 ${isDark ? 'bg-rose-600/15' : 'bg-blue-300/10'}`}
        />

        {/* ── Layer 4: Subtle horizontal glow lines ── */}
        <div className={`absolute top-[30%] left-0 right-0 h-px bg-gradient-to-r from-transparent ${isDark ? 'via-rose-500/15' : 'via-rose-500/10'} to-transparent transition-colors duration-1000`} />
        <div className={`absolute top-[70%] left-0 right-0 h-px bg-gradient-to-r from-transparent ${isDark ? 'via-blue-500/15' : 'via-blue-500/10'} to-transparent transition-colors duration-1000`} />

        {/* ── Layer 5: Vertical border/divider ── */}
        <div className={`absolute top-0 bottom-0 right-0 w-px transition-colors duration-1000 ${isDark ? 'bg-white/[0.04]' : 'bg-slate-200'}`} />
        ...
        <div className="relative z-10 flex flex-col justify-between p-12 xl:p-16 w-full h-full">
          ...
          <div className="flex flex-col gap-12 xl:gap-14 mt-12">
            ...
            <motion.div
              ...
              className="space-y-6"
            >
              <h1 className={`text-4xl xl:text-[3.75rem] font-black tracking-tight leading-[1.1] transition-colors duration-1000 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                Smart Facility{' '}
              ...
              <p className={`text-lg xl:text-xl max-w-md leading-relaxed font-medium transition-colors duration-1000 ${isDark ? 'text-slate-400' : 'text-slate-600'}`}>
              ...
              {features.map((f) => (
                <motion.div
                  ...
                  className={`group/feat flex items-center gap-5 px-6 py-4 rounded-2xl
                    transition-all duration-500 cursor-default w-fit shadow-sm
                    ${isDark 
                ...
                >
                  ...
                  >
                    <f.icon className="h-5 w-5 text-rose-500 transition-transform duration-500 group-hover/feat:scale-110" />
                  </div>
                  <div>
                    <p className={`text-base font-bold tracking-tight transition-colors duration-1000 ${isDark ? 'text-white/95' : 'text-slate-800'}`}>
                      {f.label}
                    </p>
                    <p className={`text-sm font-semibold transition-colors duration-1000 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                      {f.desc}
                    </p>
                  </div>
                </motion.div>
          ...
          >
            <div className={`h-px w-full transition-colors duration-1000 ${isDark ? 'bg-gradient-to-r from-transparent via-white/[0.08] to-transparent' : 'bg-gradient-to-r from-transparent via-slate-200 to-transparent'}`} />
            <div className="flex items-center justify-between">
              <p className={`text-xs font-bold tracking-[0.15em] uppercase transition-colors duration-1000 ${isDark ? 'text-slate-600' : 'text-slate-400'}`}>
                © 2025 SFM — Intelligence first
              </p>
              <div className="flex items-center gap-3 group/vigyani cursor-pointer">
                <span className={`text-xs font-bold transition-colors duration-1000 ${isDark ? 'text-slate-500' : 'text-slate-500'}`}>
                  Powered by
                </span>
                <div className="relative p-1.5 rounded-lg transition-all duration-500 group-hover/vigyani:scale-110">
                  <img
                    src={vigyaniLogo}
                    alt="Vigyani AI"
                    className="h-6 w-auto transition-all duration-700 group-hover/vigyani:rotate-[360deg] drop-shadow-md"
                  />
                </div>
      ...
      <div className="flex-1 flex items-center justify-center p-8 relative overflow-hidden transition-colors duration-1000 bg-transparent">
        
        {/* Right Side Theme Background */}
        <div className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${isDark ? 'opacity-0' : 'opacity-100'} bg-slate-50/50`} />
        <div className={`absolute inset-0 transition-opacity duration-1000 ease-in-out ${isDark ? 'opacity-100' : 'opacity-0'} bg-[#0d1117]/50`} />
        ...
        >
          <Button
            variant="outline"
            size="icon"
            className={`rounded-full w-12 h-12 shadow-sm hover:shadow-xl transition-all duration-500 backdrop-blur-md border-slate-200 dark:border-white/10 bg-white/50 dark:bg-black/20 ${isDark ? 'hover:shadow-rose-500/10' : 'hover:shadow-rose-500/20'}`}
            onClick={toggleTheme}
            data-testid="login-theme-toggle"
          >
            <div className="relative w-5 h-5">
              <Sun className={`absolute inset-0 w-5 h-5 text-amber-500 transition-all duration-700 ease-in-out ${isDark ? 'rotate-90 opacity-0 scale-50' : 'rotate-0 opacity-100 scale-100'}`} />
              <Moon className={`absolute inset-0 w-5 h-5 text-indigo-400 transition-all duration-700 ease-in-out ${isDark ? 'rotate-0 opacity-100 scale-100' : '-rotate-90 opacity-0 scale-50'}`} />
            </div>
          </Button>
        </motion.div>
        ...
        >
          ...
          >
            ...
            <CardHeader className="space-y-4 text-center pb-10 pt-12">
              ...
              <CardTitle className={`text-4xl font-black tracking-tight transition-colors duration-1000 ${isDark ? 'text-white' : 'text-slate-900'}`}>
                Welcome back
              </CardTitle>
              <CardDescription className={`text-base font-medium transition-colors duration-1000 ${isDark ? 'text-slate-400' : 'text-slate-500'}`}>
            ...
            <CardContent className="pb-12 px-10">
              <form onSubmit={handleSubmit} className="space-y-7">
                ...
                >
                  <Label
                    htmlFor="email"
                    className={`text-sm font-bold tracking-widest uppercase transition-colors duration-500 group-focus-within/input:text-rose-500 
