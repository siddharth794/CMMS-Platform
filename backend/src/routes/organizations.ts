...
router.post('/', async (req, res, next) => {
    try {
        ...
        const org: any = await Organization.create({ name, description, address });
...
router.get('/', async (req: any, res, next) => {
    try {
        ...
        if (req.user.Role?.name?.toLowerCase() !== 'super_admin') {
...
router.get('/:org_id', async (req: any, res, next) => {
    try {
        if (req.user.org_id !== req.params.org_id && req.user.Role?.name?.toLowerCase() !== 'super_admin') {
            res.status(403).json({ detail: 'Access denied' });
            return;
        }
